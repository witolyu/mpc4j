 ...
 * @date 2022/12/06
 */
public class Ghll22AhePsuClient extends AbstractPsuClient {
    /**
     * 核COT协议接收方
     */
    ...
    public Ghll22AhePsuClient(Rpc clientRpc, Party serverParty, Ghll22AhePsuConfig config) {
        super(Ghll22AhePsuPtoDesc.getInstance(), clientRpc, serverParty, config);
    ...
    public Set<ByteBuffer> psu(Set<ByteBuffer> clientElementSet, int serverElementSize, int elementByteLength)
        ...
        DataPacketHeader kemOvdmHeader = new DataPacketHeader(
                taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.CLIENT_SEND_OVDM_KEM.ordinal(), extraInfo,
                ownParty().getPartyId(), otherParty().getPartyId()
        );
    ...
    private void pipelinePeqt() throws MpcAbortException {
        ...
        for (round = 0; round < pipelineTime; round++) {
            ...
            DataPacketHeader reRandKemHeader = new DataPacketHeader(
                    taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.SERVER_SEND_RERAND_KEM.ordinal(), extraInfo,
                    otherParty().getPartyId(), ownParty().getPartyId()
            );
            ...
            DataPacketHeader reRandCtHeader = new DataPacketHeader(
                    taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.SERVER_SEND_RERAND_CT.ordinal(), extraInfo,
                    otherParty().getPartyId(), ownParty().getPartyId()
            );
