...
public class Ghll22AhePsuServer extends AbstractPsuServer {
    /**
     * Zn-OVDM类型
     */
...
//    private List<byte[]> ctOvdmPayload;
    ...
    public Ghll22AhePsuServer(Rpc serverRpc, Party clientParty, Ghll22AhePsuConfig config) {
        super(Ghll22AhePsuPtoDesc.getInstance(), serverRpc, clientParty, config);
...
//        coreCotSender.init(delta, maxServerElementSize);
        ...
        DataPacketHeader keysHeader = new DataPacketHeader(
                taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.SERVER_SEND_OVDM_KEYS.ordinal(), extraInfo,
        ...
        DataPacketHeader pkHeader = new DataPacketHeader(
                taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.CLIENT_SEND_PK.ordinal(), extraInfo,
    ...
    public void psu(Set<ByteBuffer> serverElementSet, int clientElementSize, int elementByteLength)
        ...
        DataPacketHeader ctOvdmHeader = new DataPacketHeader(
                taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.CLIENT_SEND_OVDM_CT.ordinal(), extraInfo,
                otherParty().getPartyId(), ownParty().getPartyId()
        );
...
//                taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.SERVER_SEND_ENC_ELEMENTS.ordinal(), extraInfo,
//                ownParty().getPartyId(), otherParty().getPartyId()
//        );
    ...
    private void pipelineReRand() {
        ...
        for (round = 0; round < pipelineTime; round++) {
            ...
            DataPacketHeader reRandCt0Header = new DataPacketHeader(
                    taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.SERVER_SEND_RERAND_CT0.ordinal(), extraInfo,
                    ownParty().getPartyId(), otherParty().getPartyId()
            );
...
//                        FIXME: Bytebuffer to bytearray, is this correct?
            ...
            DataPacketHeader reRandCt1Header = new DataPacketHeader(
                    taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.SERVER_SEND_RERAND_CT1.ordinal(), extraInfo,
                    ownParty().getPartyId(), otherParty().getPartyId()
            );
            ...
            DataPacketHeader reRandCt2Header = new DataPacketHeader(
                    taskId, getPtoDesc().getPtoId(), Ghll22AhePsuPtoDesc.PtoStep.SERVER_SEND_RERAND_CT2.ordinal(), extraInfo,
                    ownParty().getPartyId(), otherParty().getPartyId()
            );
