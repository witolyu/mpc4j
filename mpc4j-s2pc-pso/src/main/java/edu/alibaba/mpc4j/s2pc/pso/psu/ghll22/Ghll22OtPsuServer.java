...
public class Ghll22OtPsuServer extends AbstractPsuServer {
    /**
     * 核COT协议发送方
     */
    ...
    public Ghll22OtPsuServer(Rpc serverRpc, Party clientParty, Ghll22OtPsuConfig config) {
        super(Ghll22OtPsuPtoDesc.getInstance(), serverRpc, clientParty, config);
    ...
    public void init(int maxServerElementSize, int maxClientElementSize) throws MpcAbortException {
        ...
        DataPacketHeader keysHeader = new DataPacketHeader(
                taskId, getPtoDesc().getPtoId(), Ghll22OtPsuPtoDesc.PtoStep.SERVER_SEND_OVDM_KEYS.ordinal(), extraInfo,
        ...
        DataPacketHeader pkHeader = new DataPacketHeader(
                taskId, getPtoDesc().getPtoId(), Ghll22OtPsuPtoDesc.PtoStep.CLIENT_SEND_PK.ordinal(), extraInfo,
    ...
    public void psu(Set<ByteBuffer> serverElementSet, int clientElementSize, int elementByteLength)
        ...
        DataPacketHeader kemOvdmHeader = new DataPacketHeader(
                taskId, getPtoDesc().getPtoId(), Ghll22OtPsuPtoDesc.PtoStep.CLIENT_SEND_OVDM_KEM.ordinal(), extraInfo,
                otherParty().getPartyId(), ownParty().getPartyId()
        );
        List<byte[]> kemOvdmPayload = rpc.receive(kemOvdmHeader).getPayload();
        // 接收密文payload
        DataPacketHeader ctOvdmHeader = new DataPacketHeader(
                taskId, getPtoDesc().getPtoId(), Ghll22OtPsuPtoDesc.PtoStep.CLIENT_SEND_OVDM_CT.ordinal(), extraInfo,
                otherParty().getPartyId(), ownParty().getPartyId()
        );
...
//                .put(serverElementArrayList.get(0)).put(rs[0].toByteArray()).array();
        ...
        DataPacketHeader encHeader = new DataPacketHeader(
                taskId, getPtoDesc().getPtoId(), Ghll22OtPsuPtoDesc.PtoStep.SERVER_SEND_ENC_ELEMENTS.ordinal(), extraInfo,
                ownParty().getPartyId(), otherParty().getPartyId()
        );
    ...
    private void pipelineReRand(BigInteger[] rs) {
        ...
        for (round = 0; round < pipelineTime; round++) {
            ...
            DataPacketHeader reRandKemHeader = new DataPacketHeader(
                    taskId, getPtoDesc().getPtoId(), Ghll22OtPsuPtoDesc.PtoStep.SERVER_SEND_RERAND_KEM.ordinal(), extraInfo,
                    ownParty().getPartyId(), otherParty().getPartyId()
            );
            ...
            DataPacketHeader reRandCtHeader = new DataPacketHeader(
                    taskId, getPtoDesc().getPtoId(), Ghll22OtPsuPtoDesc.PtoStep.SERVER_SEND_RERAND_CT.ordinal(), extraInfo,
                    ownParty().getPartyId(), otherParty().getPartyId()
            );
